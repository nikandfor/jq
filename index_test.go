...
func TestIndexPath(tb *testing.T) {
	...
	testOnePath(tb, Index(-100), b, root, code(Null), NodePath{ps(root, -1)})
	testOnePath(tb, Index(100), b, root, code(Null), NodePath{ps(root, -1)})
}
...
func TestKeyPath(tb *testing.T) {
	...
	testOnePath(tb, Key("e"), b, root, code(Null), NodePath{ps(root, -1)})
}

func ps(off Off, i int) NodePathSeg { return NodePathSeg{Off: off, Index: i} }
