...
func TestParser(t *testing.T) {
	...
	testParser(t, `.1`, &p)
	testParser(t, `"a"`, &p)
	testParser(t, `'a'`, &Parser{SingleQuoteString: true})
	testParser(t, `.a`, &p)
	testParser(t, `$a`, &p)
	...
	testParser(t, `true and (false or false)`, &p)
	testParser(t, `qwe(. > 1, . < 2)`, &Parser{CommaSeparatedArgs: true})
	testParser(t, `qwe(. > 1, . < 2)`, &Parser{})
	testParser(t, `qwe((. > 1, . < 2))`, &Parser{CommaSeparatedArgs: true})
	testParser2(t, `qwe(. > 1; . < 2)`, `qwe(. > 1, . < 2)`, &Parser{CommaSeparatedArgs: true})
